...
/// A concrete implementation of [GetMesDonsUseCase] with parameters.
///
/// This class requires a [DonRepository] to function.
/// It calls the repository method with the given parameters.
class GetMesDonsUseCase implements UseCase<List<EntityDon>, NoParams> {
  /// Repository to interact with data layer.
  final DonRepository repository;

  /// Constructor for the use case, requiring a [repository].
  const GetMesDonsUseCase({required this.repository});
  ...
  Future<(Failure?, List<EntityDon>)> call(NoParams params) async {
    return await repository.getMesDons();
  }
}
